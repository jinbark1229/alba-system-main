...
| 기능 | 설명 |
|------|------|
| 허용이름 관리 | 직원 등록 화이트리스트 관리, 개인 인증코드 발급 |
...
**이유**: 데모 계정은 Supabase `users` 테이블에 실제 데이터로 존재하지 않는 하드코딩 계정입니다. 데이터 내보내기는 DB의 `work_logs` 테이블에서 직원 이름으로 실제 근무 기록을 조회하는데, 데모 계정으로 입력된 근무 일지가 없으면 빈 ZIP 파일이 생성됩니다.

**정상 테스트 방법**:
1. `boss` 계정으로 로그인 → 허용이름 관리에서 실제 직원 등록
2. 직원이 인증코드로 회원가입 → 근무 일지 입력
3. `boss` 계정 → 데이터 내보내기로 CSV 다운로드
...
```sql
-- 사용자 (실제 가입 계정)
users: id, name, password, role, store_id, created_at
...
│   ├── ScheduleCalendar.tsx  # 달력 뷰 (Realtime)
│   └── ScheduleTableView.tsx # 주간 표 뷰 (Realtime)
├── context/
│   └── AuthContext.tsx  # 전역 인증 상태 (로그인·권한·사용자관리)
├── pages/
│   ├── Login.tsx              # 로그인 (Supabase 직접 쿼리)
│   ├── Register.tsx           # 인증코드 기반 회원가입
│   ├── DailyLog.tsx           # 근무 일지 입력
│   ├── SchedulePage.tsx       # 근무 일정 뷰
│   ├── SalaryPage.tsx         # 급여 계산
│   ├── NoticePage.tsx         # 공지사항 페이지
│   ├── AdminExportPage.tsx    # 데이터 내보내기
...
- **실시간 동기화**: Supabase Realtime으로 공지·스케줄 즉시 반영
- **인증 보안**: 로그인·비밀번호 변경·허용이름 확인을 모두 Supabase 직접 쿼리
- **회원가입 보안**: 개인 인증코드 기반 화이트리스트 시스템으로 무단 가입 방지
- **역할 로딩 버그**: AuthContext isLoading 처리로 간헐적 접근 거부 해결
- **데이터 내보내기**: JSZip으로 직원별 CSV를 하나의 ZIP으로 패키징

---

## 📸 스크린샷

| 로그인 | 근무 일지 | 공지사항 | 허용이름 관리 |
|:---:|:---:|:---:|:---:|
| (shift-hub.vercel.app/login) | (/daily-log) | (/notices) | (/admin/allowed-names) |
